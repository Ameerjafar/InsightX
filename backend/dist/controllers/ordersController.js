...
export const openOrder = async (req, res) => {
    ...
    const { type, quantity, asset, stopLoss, takeProfit } = req.body;
    ...
    try {
        if (type === "buy") {
            ...
            const canAfford = userBalance[0].freeMargin >= parseInt(cryptoValue) * quantity;
            if (!canAfford) {
                ...
                const updateUserBalance = await prisma.balance.update({
                    ...
                    data: {
                        ...
                        freeMargin: userBalance[0].freeMargin - parseInt(cryptoValue) * quantity,
                        lockedMargin: userBalance[0].lockedMargin + parseInt(cryptoValue) * quantity,
                    },
                });
            ...
            if (!canAfford) {
                ...
                const updateUserBalance = await prisma.balance.update({
                    ...
                    data: {
                        ...
                        lockedMargin: userBalance[0].USD + parseInt(cryptoValue) * quantity,
                    },
                });
                const updateIndividualValue = await prisma.individualAsset.create({
                    data: {
...
export const closeOrder = async (req, res) => {
    ...
    const { type, quantity: quantity, asset } = req.body;
    try {
        ...
        const userIndividualAsset = userBalance.asset.find((a) => a.crypto === asset);
